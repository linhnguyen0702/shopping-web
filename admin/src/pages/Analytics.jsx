...
const Analytics = () => {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      if (!ordersRes.data?.success) throw new Error(ordersRes.data?.message || "Lỗi API đơn hàng");
      if (!usersRes.data?.success) throw new Error(usersRes.data?.message || "Lỗi API người dùng");
      if (!orderStatsRes.data?.success) throw new Error(orderStatsRes.data?.message || "Lỗi API thống kê đơn hàng");
      ...
      const conversionRate = totalUsers > 0 ? Number(((totalOrders / totalUsers) * 100).toFixed(2)) : 0;
      ...
      try {
        ...
        const usersPrev = users.filter((u) => u.createdAt && new Date(u.createdAt).getTime() >= startPrev && new Date(u.createdAt).getTime() < startCurr).length;
  ...
  const stats = [
    {
      ...
      change: `${analyticsData.revenueChangePct >= 0 ? "+" : ""}${analyticsData.revenueChangePct}%`,
    ...
    },
    {
      ...
      change: `${analyticsData.ordersChangePct >= 0 ? "+" : ""}${analyticsData.ordersChangePct}%`,
    ...
    },
    {
      ...
      change: `${analyticsData.usersChangePct >= 0 ? "+" : ""}${analyticsData.usersChangePct}%`,
  ...
  if (analyticsData.loading) {
    return (
      <div className="p-6">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Bảng điều khiển phân tích</h1>
          <p className="text-gray-600">Đang tải dữ liệu phân tích từ hệ thống...</p>
        </div>
  ...
  if (analyticsData.error) {
    return (
      <div className="p-6">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Bảng điều khiển phân tích</h1>
          <p className="text-gray-600">Có lỗi xảy ra khi tải dữ liệu từ hệ thống</p>
        </div>
        <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          <div className="mx-auto w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mb-4">
            <FaChartLine className="text-2xl text-red-600" />
          </div>
          <h3 className="text-xl font-semibold text-red-800 mb-2">Không thể tải dữ liệu phân tích</h3>
          <p className="text-red-600 mb-4">{analyticsData.error}</p>
          <button onClick={fetchAnalyticsData} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg font-medium transition-colors">Thử lại</button>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Bảng điều khiển phân tích</h1>
            <p className="text-gray-600">Dữ liệu thực tế từ hệ thống - Cập nhật lần cuối: {new Date().toLocaleString('vi-VN')}</p>
          </div>
          <button onClick={fetchAnalyticsData} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Thông tin doanh thu chi tiết</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <p className="text-sm text-gray-600 mb-1">Doanh thu trung bình/đơn hàng</p>
            <p className="text-xl font-bold text-green-600">{formatVND(analyticsData.averageOrderValue)}</p>
          </div>
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-gray-600 mb-1">Doanh thu rút gọn</p>
            <p className="text-xl font-bold text-blue-600">{formatCompactVND(analyticsData.totalRevenue)}</p>
          </div>
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <p className="text-sm text-gray-600 mb-1">Tăng trưởng đơn hàng (MoM)</p>
            <p className={`text-xl font-bold ${analyticsData.ordersChangePct >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {analyticsData.ordersChangePct >= 0 ? '+' : ''}{analyticsData.ordersChangePct}%
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
          <div>
            <p><strong>Đơn hàng:</strong> Lấy từ API `/api/order/list` và `/api/order/stats`</p>
            <p><strong>Người dùng:</strong> Lấy từ API `/api/user/users`</p>
          </div>
          <div>
            <p><strong>Doanh thu:</strong> Tính từ tổng `amount` của đơn hàng</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Biểu đồ doanh thu thực tế</h3>
          <div className="h-64 flex items-center justify-center bg-gray-50 rounded-lg">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">Hiển thị doanh thu thực tế: {formatVND(analyticsData.totalRevenue)}</p>
            </div>
          </div>
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Xu hướng đơn hàng thực tế</h3>
          <div className="h-64 flex items-center justify-center bg-gray-50 rounded-lg">
            <div className="text-center">
              <FaShoppingCart className="mx-auto text-4xl text-gray-400 mb-3" />
              <p className="text-gray-500">Biểu đồ sẽ được tích hợp ở đây</p>
              <p className="text-sm text-gray-400 mt-1">Tổng đơn hàng: {analyticsData.totalOrders.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
